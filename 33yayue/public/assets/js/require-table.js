define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template','bootstrap-table-pagejump'], function ($, undefined, Moment) {
    var Table = {
        list: {},
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                table.on('post-body.bs.table', function (e, settings, json, xhr) {
                    ...
                    //让table下的img有预览功能。 require-table.js table.on('post-body.bs.table'
                    $.each($('.table img'), function(i,val){
                        val.setAttribute("data-tips-image",val.src);
                        if(val.parentNode.nodeName.toUpperCase()=="A"){
                            val.parentNode.setAttribute("href","javascript:void(0);") ;
                            $(val.parentNode).removeAttr("target");
                            }  
                        });
            ...
            // 单元格数据格式化
            formatter: {
                maxemsurl: function (value, row, index) {
                    var l = value.length;
                    if (l>40) {
                        var newlen = value.slice(0,40)+'...';
                    }else{
                        var newlen = value;
                    }
                    var lpname = "";
                    ...
                    var html = '<span style="color:#B48443;">'+lpname+'</span><a href="javascript:;" style="color:#444;" class="btn-detail" data-id="'+row.id+'" title="' + __('%s', value) + '" data-field="' + this.field + '" data-value="' + value + '">' + newlen + '</a><span style="color:#090">[图]</span>';
                    return html;
                },
                maxems: function (value, row, index) {
                    var l = value.length;
                    if (l>20) {
                        var newlen = value.slice(0,20)+'...';
                    }else{
                        var newlen = value;
                    }
                    var html = '<a href="javascript:;" style="color:#444;" class="" data-toggle="tooltip" title="' + __('%s', value) + '" data-field="' + this.field + '" data-value="' + value + '">' + newlen + '</a>';
                    return html;
                },
